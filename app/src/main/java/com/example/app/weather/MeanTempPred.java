...
public class MeanTempPred extends AppCompatActivity implements callback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv.setText("Mean Temp Predictor");
    ...
    public String onresponse(String response) throws JSONException {
        ...
        for (int i = 0; i < jsonObject.getJSONArray("records").length() - 2; i++) {
...
//            Log.d("Band",jsonObject.getJSONArray("records").getJSONObject(i).get("year").toString());
            ...
            val = new String[2];
            val[0] = jsonObject.getJSONArray("records").getJSONObject(i).get("oct_dec_").toString();
            val[1] = jsonObject.getJSONArray("records").getJSONObject(i).get("year").toString();
            oct_dec.add(val);
        }
        jana = conva(jan);
