...
public class MaxTempView extends AppCompatActivity implements callback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv.setText("Max Temp Data");
        asynservice asynservice = new asynservice(library.Maxtempurl, this);
        asynservice.execute();
    ...
    public String onresponse(String response) {
        try {
            ...
            for (int i = 0; i < jsonObject.getJSONArray("records").length() - 4; i++) {
                Log.d("mesage", jsonObject.getJSONArray("records").getJSONObject(i).toString());
                ...
                oct_decent.add(new BarEntry(Float.parseFloat(jsonObject.getJSONArray("records").getJSONObject(i).get("oct_dec").toString()), i));
